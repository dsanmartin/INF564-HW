/*
	This implementation was taken from http://programmingtenacity.blogspot.com/2016/05/library-sort.html
	based on the article: A. Bender, Michael & Farach-Colton, Martin & A. Mosteiro, Miguel. (2006). 
	Insertion Sort is O(n log n). Theory of Computing Systems. 39. 10.1007/s00224-005-1237-z. 
	Minor changes was made to test the implementation in the homework. 
 */
#include <stdio.h>
#include <stdlib.h>
...
void libSort(int A[MAX], int N, int S[MAX2], int EPSILON) {
	...
	while(pos < N) {
		...
		for(j = 0; j < goal; j++) {
			...
			if(pos >= N) return;//That element was the last, return from the function
		}

		// ----- REBALANCE -----
		//It takes linear time. Tries to spread the elements as much as possible
		for(j = sLen-1, k = min(goal * (2 + 2 * EPSILON), (1 + EPSILON) * N ) - 1, step = (k + 1) / (j + 1); j >= 0; j--, k -= step) {
			S[k] = S[j];
			S[j] = NONE;
		}
...
void librarySort(int A[MAX], int n) {
	...
	// Main process O(n log n)
	libSort(A, n, S, epsilon);

	// Linear time in remove gaps
